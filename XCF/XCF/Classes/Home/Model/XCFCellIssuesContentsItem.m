...
- (CGFloat)cellHeight{
    ...
    switch (self.cellTemplate) {
        ...
        case 5:{
            ...
            CGSize titleMaxSize = CGSizeMake(ScreenWidth - 2 * XCFMargin - XCFAuthorIconWH, MAXFLOAT);
            ...
            _cellHeight += [self.desc boundingRectWithSize:titleMaxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:15.0]} context:nil].size.height + XCFMargin;
            NSString *cookedStr = [NSString stringWithFormat:@"%@-%ld",self.score, self.n_cooked];
            _cellHeight += [cookedStr boundingRectWithSize:titleMaxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:13.0]} context:nil].size.height + XCFMargin;
            
            break;
        }
        case 6:{
            
            if (self.image.height > ScreenHeight) {
                
                _cellHeight += ScreenHeight * 0.8;
            }else{
                
                _cellHeight += self.image.width / ScreenWidth * self.image.height;
            }
